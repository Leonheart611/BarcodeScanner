...
import kotlinx.android.synthetic.main.dor_post_bottom_dialog.*
...
import kotlinx.android.synthetic.main.peminjam_post_bottom_dialog.*
...
class HomePostAllDialog : BottomSheetDialogFragment() {
	...
	private fun setupView() {
		...
		btn_dismis_dor_post.gone()
		btn_dismis_peminjam_post.gone()
	}
	...
	private fun setObseverable() {
		viewModel.homePostViewState.observe(viewLifecycleOwner, {
			when (it) {
				...
				is HomeViewModel.HomePostViewState.ErrorPostDor -> {
					iv_status_post_dor.crossFade(
						animateDuration.toLong(),
						pb_dor_post_dialog
					)
					iv_status_post_dor.setImageDrawable(
						ResourcesCompat.getDrawable(
							resources,
							R.drawable.ic_error_circle,
							null
					...
					tv_error_dor_post.text = it.message
				}
				is HomeViewModel.HomePostViewState.GetSuccessfulDor -> {
					tv_dor_posted_count.text = it.data.toString()
				}
				is HomeViewModel.HomePostViewState.GetUnpostedDor -> {
					tv_dor_total_post.text = it.data.toString()
				}
				HomeViewModel.HomePostViewState.SuccessPostallDor -> {
					iv_status_post_dor.crossFade(
						animateDuration.toLong(),
						pb_dor_post_dialog
					)
				...
				is HomeViewModel.HomePostViewState.ErrorPostPeminjam -> {
					iv_status_post_peminjam.crossFade(
						animateDuration.toLong(),
						pb_peminjam_post_dialog
					)
					iv_status_post_peminjam.setImageDrawable(
						ResourcesCompat.getDrawable(
							resources,
							R.drawable.ic_error_circle,
							null
					...
					tv_error_peminjam_post.text = it.message
				}
				is HomeViewModel.HomePostViewState.GetSuccessfulPeminjam -> {
					tv_peminjam_posted_count.text = it.data.toString()
				}
				is HomeViewModel.HomePostViewState.GetUnpostedPeminjam -> {
					tv_peminjam_total_post.text = it.data.toString()
				}
				HomeViewModel.HomePostViewState.SuccessPostallPeminjam -> {
					iv_status_post_peminjam.crossFade(
						animateDuration.toLong(),
						pb_peminjam_post_dialog
					)
